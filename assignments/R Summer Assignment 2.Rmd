...
**Using AI while learning R:** AI tools can help explain error messages or
remind you what a command does. They are most useful after you have tried the
code yourself. Always run the code, inspect the output, and make sure the result
...
# This sets a reliable package download location for Posit Cloud or RStudio.
...
For data work, use this basic workflow throughout the assignment:

1.  **Inspect** the data: What are the rows, columns, and values?
2.  **Ask** a clear question: What subset, summary, or transformation do you
    need?
3.  **Transform** the data with code.
4.  **Check** the result: Does the output have the rows, columns, and values you
    expected?
...
Two other useful inspection functions are `glimpse()` and `nrow()`. `glimpse()`
shows the columns and their types, while `nrow()` tells you how many rows are in
the dataset.
...
3.  Use `glimpse()` and `nrow()` on `population`. What does one row represent?
...
You can also use `distinct()` to ask for the unique values of a column while
staying inside the tidyverse style:

```{r}
population |> distinct(country)
```
...
Before analyzing a new dataset, check its structure and unit of analysis.

```{r}
glimpse(elections)
nrow(elections)
count(elections, region)
```
...
2.  Use `filter()` to subset results for Massachusetts and store the answer in
    an object called `ma`.
...
5.  Explain why the following code returns no rows (it may be helpful to look at
    the dataset with `elections`):
...
-   Inspect a dataset with functions like `head()`, `glimpse()`, and `nrow()`.
-   Explain what one row represents in a dataset.
-   Use `filter()`, `arrange()`, and `mutate()` to transform data.
-   Read a CSV file from a URL with `read_csv()`.
-   Check whether your output has the rows and columns you expected.
