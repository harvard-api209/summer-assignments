---
title: "R Summer Assignment -- RMarkdown 2"
...
**RMarkdown Notebook 2: Data Manipulation: Part 1**
...
1.  **Creation**: First, you must make a personal copy of this notebook. To do
    that, please go to **File → Save a Copy in Drive.**

2.  **Preparation**: Like the first notebook, we ask you to follow this RStudio
    tutorial before starting this notebook. We strongly recommend taking notes
...
-   [Manipulating Data with dplyr](https://rstudio.cloud/learn/primers/2.2)

3.  **Submission**: After making your own copy, complete your assignment in
...
Finally, if you leave the notebook for a while, you may need to reload it and
re-run any setup code. This will not be a problem once you're on campus and have
R installed on your own computer, but it can be a temporary issue with
cloud-based notebooks. If you leave the notebook for a while and come back,
please re-run any necessary setup code.

If you get stuck in any aspect of using these Posit Cloud RMarkdowns, please use
Slack to ask the teaching team.

> Please run this code first. It may take a minute to download the data It's
> okay that this code looks mysterious for now, it is here to handle
> installation for you in the background. It will make more sense soon!
...
You can type the name of the object to see these values and
also use functions on them just like before: 
...
However, what if you want information on population **and** year 
**in a single object**? This is what **datasets** are for.
...
`library(tidyverse)`. Go to the console in Posit Cloud (or RStudio) 
(the panel called "Console" with the `>` character at the bottom) and type 
`population`. It will print out a dataset that looks like this: 
...
You can do many things with datasets. Often, we will want to
use functions on datasets. For example, the `slice()` function takes two
arguments, an object and a number `n`, and the function will return to you the
first `n` rows you asked for.
...
You can also pass a vector to `slice()` to have it return
multiple rows. For example, to have `slice()` return rows 2, 3, and 4, we can
write: `2:4`.
...
Keep in mind, you can also get the same results by using the following code style:
...
When checking equality with operators like `==`, R is very specific and case-sensitive. That is, "France" is
not equal to "france," or "FRANCE."
...
Because these are not equal, when using `filter()` you need to be precise and make sure your
comparisons reflect the values in your actual data. For example, this code would
not return any rows because no rows in our dataset have country equal to
"FRANCE": 
...
R returned an empty dataset. This is not a problem with R, since it is doing exactly what we asked
it to do -- return any rows with "FRANCE" in the country column. However,
because there were no such rows, we get an empty tibble (0 rows by 3 columns).
...
Notice that FRANCE is not in there, but France is. When using `filter()`, we could target any one of these values above. More
broadly, we can `filter()` on any condition that evaluates to `TRUE` or `FALSE`.
For example, let's get data from the year 1995. 

```{r}
population |> filter(year == 1995)
```

Finally, it is often useful to use **multiple** logical conditions at
once. For example, return rows in France AND in the year 1995, or return any
data from France OR India.
...
By including the `desc()` (descending) function inside of `arrange()`, you can have it sort from largest to smallest.

For example, this code shows us that the populaion of Tuvalu keeps increasing
over time. 
...
# Write your code here
```
2.  Use `filter()` to access the rows in `population` with less than 5000
    people. Assign the result to an object called `small_countries`.
```{r}
# Write your code here
```
3.  Use `filter()` to access the rows in `population` for every country
    **other** than Greece. **Hint**: remember in Notebook 1 we talked about a
    way to check if one value is NOT equal to another value.
```{r}
# Write your code here
```
...
```
In order to keep changes in your dataset object, you will need create a new object just like always. That is,
functions like `mutate()` and `filter()` will create output for you, but will
not save these results to a new object unless you manually ask it to by creating
a new object with `<-`.
...
In general, the best practice is to write your code, make sure it is making the changes you want to your dataset,
and then save it back to an object as the last step. However, make sure you whether or not you would like to overwrite your object. Sometimes it might be better just to save your data manipulation into a new object.
...
You can make new columns to accomplish all sorts of tasks. For example, beyond checking logical statements we can do mathematical operations on
an existing column: 
...
1.  Look at the `elections` dataset with `head()`. What do the rows and columns represent?
```{r}
# Write your code here
```
...
# Write your code here
```
5.  Explain why this code below returns no rows (it may be helpful to look at
    the dataset with `elections`): elections \|\> filter(state == "MA") Write
    your answer here (double-click to add text): \## Reminder to Submit

Please follow the submission instructions listed
[here](TO BE INCLUDED). We suggest you submit your assignments as you finish them (i.e., don’t wait until you
