...
clarify what a command does, but they cannot replace your own practice. If you
use AI while working on these assignments, run the code yourself, check that the
output matches the question, and be ready to explain the result in your own
words.
...
Here are a few words we will use throughout these assignments:

-   **Object**: a named thing that stores a value for later use.
-   **Function**: a command that performs a task, usually using values inside
    parentheses.
-   **Argument**: an input you give to a function.
-   **Vector**: an object that stores more than one value.
...
6.  Read the error message from question 5 carefully. In one sentence, explain
    what R tried to do and why it could not do it. If you use AI to help explain
    the error, write the explanation in your own words and say how you checked
    that it matched what happened in R.
...
-   Run simple R code and read the output.
-   Create objects with `<-`.
-   Recognize numbers, characters, logical values, and vectors.
-   Use basic functions and read simple documentation pages.
-   Read an error message carefully before asking for help.
