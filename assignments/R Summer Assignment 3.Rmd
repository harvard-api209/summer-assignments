...
**Using AI while learning R:** AI tools can sometimes suggest `ggplot()` code,
but they may use column names that are not in your dataset or produce a graph
that does not answer your question. Treat any suggestion as a draft: run it,
check the mappings, and explain what the graph shows.
...
ma |>
  ...
  labs(
    ...
    title = "Massachusetts Presidential Elections",
    subtitle = "Democratic vote share, 1932-2016"
  )
...
ma |>
  ...
  labs(
    ...
    y = "Democratic vote share",
    title = "Massachusetts Presidential Elections",
    subtitle = "Points show individual elections"
  )
```
...
ma |>
  ...
  geom_line(color = hks_gray) +
  geom_point(color = hks_blue) +
  labs(
    x = "Election year",
    y = "Democratic vote share",
    title = "Massachusetts Presidential Elections",
    subtitle = "Line and points show the same values in different ways"
  )
```
...
When judging a graph, ask yourself: What is the main comparison? What does each
axis represent? Are color, size, or shape adding information? What should a
reader conclude after looking at the figure for ten seconds?
...
ma |>
  ...
  geom_point(size = 2.4) +
  scale_color_manual(values = c("FALSE" = hks_crimson, "TRUE" = hks_blue)) +
  labs(
    x = "Election year",
    y = "Democratic vote share",
    color = "Democratic win",
    title = "Color Mapped to the Data"
  )
```
...
ma |>
  ...
  labs(
    ...
    y = "Democratic vote share",
    color = "Democratic win",
    title = "When Color Affects Both Points and Lines"
  )
```
...
ma |>
  ...
  geom_line(color = hks_gray) +
  geom_point(aes(color = winner), size = 2.4) +
  scale_color_manual(values = c("FALSE" = hks_crimson, "TRUE" = hks_blue)) +
  labs(
    x = "Election year",
    y = "Democratic vote share",
    color = "Democratic win",
    title = "Color Only the Points"
  )
```
...
ma|>
  ...
  labs(
    ...
    y = "Democratic vote share",
    color = "Democratic win",
    title = "Fixed Line Color, Data-Driven Point Color"
  )
```
...
ma|>
  ggplot(aes(x = year, y = democrat)) +
    geom_line(color = "gray75") +
    geom_point(aes(size = winner), color = hks_blue) +
    labs(
      x = "Election year",
      y = "Democratic vote share",
      size = "Democratic win",
      title = "Size Mapped to a Logical Value"
    )
```
...
ma|>
  ...
  geom_line(color = "gray75") +
  geom_point(aes(shape = winner), color = hks_blue, size = 2.4) +
  labs(
    x = "Election year",
    y = "Democratic vote share",
    shape = "Democratic win",
    title = "Shape Mapped to a Logical Value"
  )
```
...
function, which has arguments for `title`, `subtitle`, `caption`, `x`, and `y`
...
ma |>
  ...
  geom_point(aes(color = winner), size = 2.4) +
  scale_color_manual(values = c("FALSE" = hks_crimson, "TRUE" = hks_blue)) +
  labs(title = "Massachusetts Presidential \n Election Results",
       subtitle = "1932-2016",
       x = "Election Year",
       y = "Democratic %",
       color = "Democratic win") +
...
p <- ma |>
  ...
  scale_color_manual(values = c("FALSE" = hks_crimson, "TRUE" = hks_blue)) +
  labs(
    title = "Massachusetts Presidential Election Results",
    subtitle = "1932-2016",
    x = "Election Year",
    y = "Democratic %",
    color = "Democratic win"
...
northeast |>
  ...
  geom_point(color = hks_blue, alpha = 0.45) +
  labs(
    x = "Election year",
    y = "Democratic vote share",
    title = "Northeast Presidential Election Results",
    subtitle = "All Northeastern states shown together"
  )
```
...
northeast |>
  ...
  labs(
    ...
    y = "Democratic vote share",
    color = "State",
    title = "Coloring Every State Can Become Busy"
  )
```
...
northeast |>
  ...
  labs(
    ...
    y = "Democratic vote share",
    color = "State",
    title = "Many Lines Can Still Be Difficult to Read"
  )
```
...
northeast |>
  ggplot(aes(x = year, y = democrat)) +
  geom_line(color = hks_gray) +
  geom_point(color = hks_blue, size = 1.8) +
  facet_wrap(~state) + # notice the ~ key (called a tilde)
  labs(
    x = "Election year",
    y = "Democratic vote share",
    title = "Facets Separate the States"
  )
```
...
northeast |>
  ...
  geom_line(color = "gray75") +
  geom_point(aes(color = winner), size = 1.8) +
  scale_color_manual(values = c("FALSE" = hks_crimson, "TRUE" = hks_blue)) +
  facet_wrap(~state) + # notice the ~ key (called a tilde)
  labs(
    x = "Election Year",
...
northeast |>
  ...
  labs(
    x = "Election year",
    y = "Democratic vote share",
    title = "Adjusting Text Size in Facets"
  ) +
  theme(
...
4.  Optional AI check: if you use an AI tool to critique one of your plots, list
    one suggestion it made. Did you accept or reject that suggestion? Explain
    your reasoning by referring to the data, the code, or the plot.
...
-   Build a plot with `ggplot()`, `aes()`, and a geom.
-   Decide when a line, point, bar, or faceted plot is useful.
-   Use color, size, and shape intentionally.
-   Add labels and themes that make a plot easier to read.
-   Critique whether a plot answers the question being asked.
