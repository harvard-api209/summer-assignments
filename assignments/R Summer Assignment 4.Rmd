...
**Using AI while learning R:** AI tools can help you remember syntax for
`group_by()`, `summarise()`, or a graphing function. They cannot decide whether
your denominator, grouping level, or interpretation is correct. Always check
what one row represents before and after you summarize.
...
# This code sets a simple plot style for the examples below.
...
update_geom_defaults("point", list(size = 2.2, alpha = 0.8))
...
state |>
  ...
  geom_point(color = hks_blue, alpha = 0.45, size = 1.4) +
  scale_y_continuous(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(
    x = "Year",
    y = "State population",
    title = "State Populations Over Time",
    subtitle = "Each point is one state in one year"
  )
```
...
Notice that the unit of analysis changed. In `state`, one row is a state-year.
In `pop_by_year`, one row is a year. This is why checking the rows after
`summarise()` is so important.
...
The function `n()` counts how many rows are in each group. This is useful for
checking whether each summary is based on the number of observations you
expected.
...
pop_by_year |>
  ...
  geom_col(fill = hks_blue, width = 0.85) +
  scale_y_continuous(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(
    x = "Year",
    y = "Total population",
    title = "US Population by Year",
    subtitle = "Total population across states in the dataset"
  )
```
...
pop_by_year |>
  ...
  geom_segment(aes(x = year, xend = year, y = 0, yend = pop), color = "gray80") +
  geom_point(color = hks_crimson, size = 2.4) +
  scale_y_continuous(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(
    x = "Year",
    y = "Total population",
    title = "US Population by Year",
    subtitle = "A lollipop plot draws attention to the top of each value"
  )
```
...
region_year |>
  ...
  geom_point(color = hks_blue, size = 1.2) +
  scale_y_continuous(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(
    x = "Year",
    y = "Total population",
    title = "US Population by Region",
    subtitle = "Each panel summarizes state populations within a region"
  ) +
  facet_wrap(~region)
```
...
In one sentence, describe the main finding from your plot. Then write one
sentence about whether the plot makes comparison across teams easy or difficult.
...
Be careful with averages. The average of team-level rates is not always the same
as the overall rate. In the code below, the unweighted mean treats every team
equally, while the weighted rate gives more influence to teams with more
matches.
...
cups |>
  filter(year == 1930) |>
  summarise(
    unweighted_goals_per_match = mean(scored / matches),
    weighted_goals_per_match = sum(scored) / sum(matches)
  )
```

If an AI tool suggests a summary statistic, check whether it is using the right
level of aggregation and the right denominator.
...
When summarizing real datasets, also check for missing values before trusting a
mean or median.

```{r}
sum(is.na(cups$scored))
```
...
cups |>
  ...
  geom_histogram(binwidth = 5, fill = hks_blue, color = "white", linewidth = 0.2) +
  labs(
    x = "Shots on goal",
    y = "Number of team-World Cup observations",
    title = "Shots on Goal Since 1950",
    subtitle = "Most teams are concentrated at lower shot totals"
  )
```
...
cups |>
  ...
  labs(
    x = "Shots on goal",
    y = "Count",
    title = "Shots on Goal by World Cup Year"
  ) +
  facet_wrap(~year)
```
...
cups |>
  filter(year > 1950) |>
  ggplot(aes(x = shots_on_goal)) +
  geom_histogram(binwidth = 1, fill = hks_blue, color = "white", linewidth = 0.15) +
  labs(
    x = "Shots on goal",
    y = "Count",
    title = "Histogram With a Small Binwidth",
    subtitle = "More detail, but also more visual noise"
  )
```
...
cups |>
  filter(year > 1950) |>
  ggplot(aes(x = shots_on_goal)) +
  geom_histogram(binwidth = 25, fill = hks_blue, color = "white", linewidth = 0.2) +
  labs(
    x = "Shots on goal",
    y = "Count",
    title = "Histogram With a Large Binwidth",
    subtitle = "Less detail, which can hide important variation"
  )
```
...
`?geom_vline` or by Googling "R `geom_vline()`, then scroll down to"Examples" to
see code examples using the function).

Finally, explain the general trend that you see.

In your explanation, describe the center, spread, skew, and any visible outliers
or regional differences. If changing the binwidth changes your conclusion, say
so.
...
Here, in text describe the trend you see over time!
...
-   Use `group_by()` and `summarise()` to calculate grouped statistics.
-   Explain how summarizing changes the unit of analysis.
-   Use `n()` to check how many rows are in each group.
-   Compare means and medians, including when outliers matter.
-   Describe the center, spread, skew, and outliers in a distribution.

## Before you submit

-   Knit the notebook successfully.
-   Answer every written question in your own words.
-   Save your completed `.Rmd` file.
-   Submit the correct `.Rmd` file to Canvas.
