...
pkgs <- c("tidyverse", "ggrepel", "gapminder", "scales", "ggtext")
...
# Write your code here!
weo |>
  arrange(continent, desc(rgdp2019)) |>
  select(continent, country, rgdp2019)
```
...
weo_percap <- weo |>
  mutate(
    gdp_percap_2019 = rgdp2019 / pop2019,
    gdp_percap_1994 = rgdp1994 / pop1994,
    growth_2019_1994 = (gdp_percap_2019 - gdp_percap_1994)/gdp_percap_1994
  )
```
...
geom_point() +
labs(
  x = "GDP per capita in 1994",
  y = "GDP per capita in 2019",
  color = "Continent"
)
```
...
weo_percap |>
  summarize(
    mean_gcap_2019 = mean(gdp_percap_2019),
    median_gcap_2019 = median(gdp_percap_2019)
  )
```
...
weo_percap |>
  summarize(
    mean_gcap_2019 = mean(gdp_percap_2019, na.rm = TRUE),
    median_gcap_2019 = median(gdp_percap_2019, na.rm = TRUE)
  )
```
...
```{r}
# Write your code here!
# only use large enough countries
ds_large <- weo_percap |> 
  filter(pop1994 > 5 | pop2019 > 5)

# subset to show country names. Only use the top 5.
sub_show <- ds_large |>
  mutate(abs_change = abs(gdp_percap_2019 - gdp_percap_1994)) |> 
  arrange(desc(abs_change)) |>
  select(country, gdp_percap_2019, gdp_percap_1994) |> 
  slice(1:5)

# Create the graph, but add a layer of geom_text_repel and input the subset there
ds_large |> 
  ggplot(
    aes(
      x = gdp_percap_1994, 
  ...
  geom_point(aes(size = pop2017), alpha = 0.5) + coord_equal() +
  geom_text_repel(data = sub_show, aes(label = country), size = 7) + 
  scale_x_continuous(labels = dollar) + 
  scale_y_continuous(labels = dollar) +
  # turns off legend for size
  guides(size = FALSE) +
  labs(
    title = "**Figure 1**: Changes in GDP per capita between 1994 and 2019",
    x = "1994 GDP per Capita",
    y = "2019 GDP per Capita",
    caption = "Points sized by 2019 population and labels show top 5 countries\nwith the most absolute change. Only countries with population at least 5 million\nin 1994 or 2019 shown. Dotted line indicates 45-degree line."
  ) + 
  # optional setting to make caption text a little smaller
  theme(
